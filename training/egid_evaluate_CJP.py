...
def evaluate_egid():
  ...
  # Define input ntuple
  f_in_name = inputFiles[ opt.sampleType ] #"/afs/cern.ch/work/j/jheikkil/tools/ntuple-tools/ele23_copy.root"#"%s/cl3d_selection/%s/%s_%s_%s.root"%(os.environ['HGCAL_L1T_BASE'],opt.sampleType,opt.sampleType,opt.clusteringAlgo,opt.dataset)
  ...
  t_in = f_in.Get( treeMap[ opt.sampleType ] )
  ...
  for cl3d in t_in:
    ...
    for b in bdt_list:
      ...
      elif(abs(cl3d.eta) > eta_regions['high'][0])&(abs(cl3d.eta) <= eta_regions['high'][1]):
